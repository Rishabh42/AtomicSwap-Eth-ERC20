...
contract('AtomicSwap', (accounts) => {
  ...
  const etherValue = 500000000000000000;
  const erc20Value = 1*10;
  ...
  const swap = await atomicSwap.deployed();
  const token = await testToken.deployed();
  await token.transfer(bob, 1*10, {from: accounts[0]})
  await token.approve(swap.address, 1*10, {from: bob});
  const allowance = await token.allowance(bob,swap.address);
  assert.equal(1*10,allowance)
